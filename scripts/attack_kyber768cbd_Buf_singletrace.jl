...
TracesDIR = joinpath(@__DIR__, "../data/Traces-O1/")
TMPFILE   = joinpath(@__DIR__, "../data/", "Kyber768cbd.jl.tmp")
numproc   = Sys.CPU_THREADS÷4  # Number of multi-process for EM-adj
skipexist = true
###

tgtlist, tpllist, pooltpllist = deviceslist,deviceslist,devpoolsidx #deviceslist, deviceslist, devpoolsidx
tplDir  = DirHPFO1  # DirHPFnew
tgtDir  = DirHPFO1  # DirHPFnew
...
HW = count_ones
fbufs1(buf) = HW(bufx1(buf))-HW(bufy1(buf))
bufs1 = OffsetArray([fbufs1(b) for b in 0:255],0:255)
fbufs2(buf) = HW(bufx2(buf))-HW(bufy2(buf))
bufs2 = OffsetArray([fbufs2(b) for b in 0:255],0:255)
