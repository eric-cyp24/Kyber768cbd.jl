...
tgtlist, tpllist, pooltpllist = [:FN1, :FN2],[:FN1, :FN2],[]#deviceslist, deviceslist, devpoolsidx
...
function CBD_TA_margin!(s_guess::AbstractVector, trace::AbstractVector, tXY::Vector{Template}; prob::Bool=false)
    # Template Attack
...
function CBD_TA_margin(traces::AbstractMatrix, tXY::Vector{Template}; prob::Bool=false)
    s_guess = Vector{Int16}(undef, length(tXY)*size(traces,2))
    @sync Threads.@threads for i in 1:size(traces,2)
        @views CBD_TA_margin!(s_guess[16*i-15:16*i], traces[:,i], tXY; prob)
    end
    return s_guess
end

"""
    singletraceattacks(Traces::AbstractArray, tXY::Vector{Template}; S_true::AbstractMatrix=[;;], showprogress::Bool=true)

"""
function singletraceattacks(Traces::AbstractArray, tXY::Vector{Template}; S_true::AbstractMatrix=[;;], showprogress::Bool=true)
    S_guess = []
    if isempty(S_true)
        for (n,trace) in enumerate(eachslice(Traces,dims=3))
            showprogress && print(" ",n,"  \r")
            push!(S_guess, CBD_TA_margin(trace, tXY))
        end
    else
        for (n,trace) in enumerate(eachslice(Traces,dims=3))
...
function Cross_Device_Attack(Templateidx::Symbol, Targetidx::Symbol, postfix::AbstractString; resulth5overwrite::Bool=false, method=:marginalize,
    ...
    tXY = begin
        ...
        txyfile    = joinpath(TemplateDIR, "Templates_XY_proc_nicv$(string(nicvth)[2:end])_POIe$(POIe_left)-$(POIe_right)_lanczos2.h5")
        [loadtemplate(txyfile; byte) for byte in 1:16]
    end
    println("Done!")
    ...
    if TracesNormalization
        ...
        Traces = tracesnormalize(Traces, tXY[1]; TMPFILE)
        println("Done!")
    end
    ...
    if EMadjust
        ...
        emadjsecs = @elapsed begin
            Templates_EMadj!(tXY, :XY, Traces; num_epoch, newprocs=false)
        end
    ...
    resultfile   = "$(method)_XY_Result_with_Templates_POIe$(POIe_left)-$(POIe_right)_from_$(replace(TemplateDir,"/"=>"_")[1:end-1]).h5"
    ...
    writeTemplates(outfile, joinpath(h5resultpath, "Templates/"); tXY)
    println("Done!")

    # test Template -> write s_guess, Successrate, total/single-trace
    begin println("Single-Trace Attack...          ")
    attacksecs = @elapsed begin
        S_guess = singletraceattacks(Traces, tXY; S_true, showprogress=true)
    end
...
function CBDTemplates_EMadj!(CBDTemplates, Traces::AbstractArray; newprocs::Bool=true,
    ...
    newworkers = newprocs ? emalg_addprocs(Sys.CPU_THREADS÷2) : []
    for (iv, tIV) in zip([:Buf, :X, :Y, :S, :XY], CBDTemplates)
...
function main()
    ...
    for tgtidx in tgtlist
        ...
        if skipexist
            resultfile   = "$(method)_XY_Result_with_Templates_POIe$(POIe_left)-$(POIe_right)_from_$(replace(tplDir[tplidx],"/"=>"_")[1:end-1]).h5"
        ...
        if skipexist
            resultfile   = "$(method)_XY_Result_with_Templates_POIe$(POIe_left)-$(POIe_right)_from_$(replace(pooledDir(devicespools[tplidx]),"/"=>"_")[1:end-1]).h5"
            TargetDIR    = joinpath(TracesDIR, tgtDir[tgtidx], "lanczos2_25$(postfix)/")
            OUTDIR       = joinpath(TargetDIR, "Results/Templates_POIe$(POIe_left)-$(POIe_right)/")
            isfile(joinpath(OUTDIR, resultfile)) && continue
        end
